...
class AddTaskViewModel extends Cubit<AddTaskState> {
  ...
  AddTaskViewModel({required TaskRepository repository})
  ...
  void addTask({
    ...
    final task = AppTask.create(
      ...
      isToday: isToday,
      dueDate: timeOfDay?.toDateTime(),
    );

    final result = await _repository.saveTask(task);

    result.fold(
      (l) => emit(AddTaskFail(errorMessage: l.error)),
      (r) => emit(AddTaskSuccess(task: r)),
    );
  }
  ...
  void scheduleNotification(AppTask task) async {
    ...
    await NotificationService.cancel(task.getNotificationId());
